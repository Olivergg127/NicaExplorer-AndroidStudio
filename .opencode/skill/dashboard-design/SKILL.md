...
description: >
  Use when designing, refining, or implementing the NicaExplorer admin dashboard or
  panel (dashboard, panel administrativo, backend, AdminLTE, sidebar, cards, DataTables,
  modals, formularios, empty states, dark UI, design_dash.md). Produces a modern,
  minimalist, premium interface with clear hierarchy and NicaExplorer's dark
  green/turquoise identity, avoiding generic Bootstrap and "AI-generated UI" looks.
  Trigger on requests to redesign, refine, or build the dashboard/panel, or when
  design_dash.md is referenced.
---
